...
def get_universe(seeds, neighbors):
    universe = set(seeds) # a set containing the initial seeds
    for seed in universe.copy():
        universe |= neighbors[seed] 
    for candidate in universe.copy(): # seeds + candidates
        universe |= neighbors[candidate]
    return universe
...
def diamond_iteration_of_first_X_nodes(G, S, X, alpha):
    ...
    Universe at each iteration is made of:
    ---> seed sed + candidate nodes (neighbors of seed set) + neighbors of the candidate nodes
    ...
    # ------------------------------------------------------------------
    # Setting initial set of nodes in DIAble universe
    # + seed sed 
    # + candidate nodes (neighbors of seed set) 
    # + neighbors of the candidate nodes
    # ------------------------------------------------------------------
    universe = get_universe(S, neighbors)
    N = len(universe)
    N += (alpha - 1) * s0
    ...
    while len(added_nodes) < X:
        ...
        # Updating the list of cluster nodes, s0 and N and log functions
        cluster_nodes.add(next_node)
        s0 = len(cluster_nodes)
        N = len(get_universe(cluster_nodes, neighbors))
        N += (alpha - 1) * s0
        gamma_ln = compute_all_gamma_ln(N + 1)
...
def DIAble(G_original, seed_genes, max_number_of_added_nodes, alpha, outfile=None):
    ...
    if outfile is not None:
        with open(outfile, 'w') as fout:
            ...
            fout.write('\t'.join(['#rank', 'DIAble_node', 'p_hyper', 'universe_len']) + '\n')
            rank = 0
            for DIAble_node_info in added_nodes:
                rank += 1
                DIAble_node = DIAble_node_info[0]
                p = float(DIAble_node_info[3])
                univ_len = int(DIAble_node_info[4])

                fout.write('\t'.join(map(str, ([rank, DIAble_node, p, univ_len]))) + '\n')
